...
__global__ void find_grid_loc_gpu(float *points, int *grid_loc, int n, int d,
                                  int k) {
...
__device__ void search_block_reducer(int *closest, float *current_min,
                                     int block_offset, float *q, float *block,
                                     int points_in_block) {
  __syncthreads();
  ...
  for (int i = thread_id; i < points_in_block; i += thread_num) {
    ...
    if (dist < temp) {
      temp = dist;
      temp_index = i + block_offset;
    }
  }
  min_cashe[thread_id] = temp;
  close_cashe[thread_id] = temp_index;
  ...
  if (threadIdx.x == 0) {
    *closest = close_cashe[0];
    *current_min = min_cashe[0];
  }
}

__global__ void search_gpu(float *q, float *c, int *grid, int *points_per_block,
                           int *closests, float *mindists, int n, int d,
                           int dd) {
  // int thread_id = blockIdx.x*blockDim.x + threadIdx.x;
  ...
  for (int i = blockIdx.x; i < n; i += gridDim.x) {
    ...
    search_block_reducer(&closests[i], &mindists[i], grid[grid_loc],
                         &q[i * DIM], &c[grid[grid_loc] * DIM],
    ...
    while (1) {
      ...
      finished = __syncthreads_or(finished);
        ...
        for (int dx = x - stage; dx <= x + stage; dx++) {
          for (int dy = y - stage; dy <= y + stage; dy++) {
            for (int dz = z - stage; dz <= z + stage; dz++) {
              ...
              if (dx == x - stage || dx == x + stage || dy == y - stage ||
                  dy == y + stage || dz == z - stage || dz == z + stage) {
                grid_loc = dx + d * dy + dd * dz;
                if (dx > -1 && dx < d && dy > -1 && dy < d && dz > -1 && dz < d)

                  search_block_reducer(
                      &closests[i], &mindists[i], grid[grid_loc], point,
...
void search_block(int *closest, float *current_min, int block_offset, float *q,
...
void search(float *q, float *c, int *grid, int *points_per_block, int *closests,
          ...
          for (int dy = y - stage; dy <= y + stage; dy++) {
            for (int dz = z - stage; dz <= z + stage; dz++) {
              ...
              if (dx == x - stage || dx == x + stage || dy == y - stage ||
                ...
                if (dx > -1 && dx < d && dy > -1 && dy < d && dz > -1 && dz < d)
                  search_block(&closests[i], &mindists[i], grid[grid_loc],
                               point, &c[grid[grid_loc] * DIM],
                               points_per_block[grid_loc]);
              }
            }
          }
...
float *rearrange(float *p, int *intex, int *points_per_block, int *grid, int n,
                 int k) {
...
void validate_search(const float *q, const float *c, const int *closest, int N,
  ...
  for (int i = 0; i < VALIDATE; i++) {
    ...
    for (int j = 0; j < N; j++) {
      dist =
          (q[i * DIM + 0] - c[j * DIM + 0]) * (q[i * DIM + 0] - c[j * DIM + 0]);
      dist +=
          (q[i * DIM + 1] - c[j * DIM + 1]) * (q[i * DIM + 1] - c[j * DIM + 1]);
      dist +=
          (q[i * DIM + 2] - c[j * DIM + 2]) * (q[i * DIM + 2] - c[j * DIM + 2]);
      dist = sqrtf(dist);
      if (dist < mindist) {
        close = j;
        mindist = dist;
