...
__device__ void
search_block_reducer (int *closest, float *current_min, int block_offset,
                      float *q, float *block, int points_in_block,
                      float *min_cashe, int *close_cashe)
{
  ...
  for (int i = blockIdx.x; i < n; i += gridDim.x)
    {
      ...
      search_block_reducer (&closests[i], &mindists[i], grid[grid_loc],
                            ...
                            points_per_block[grid_loc], min_cashe, close_cashe);
                              ...
                              || dz == z - stage || dz == z + stage)
                            {
                              ...
                              if (dx > -1 && dx < d && dy > -1 && dy < d
                                ...
                                search_block_reducer (
                                  &closests[i], &mindists[i], grid[grid_loc],
                                  point, &c[grid[grid_loc] * DIM],
                                  points_per_block[grid_loc], min_cashe,
                                  close_cashe);
                            }
                        }
                    }
...
write_file (double time_var, const char *filename, const char *mode)
{
  FILE *fptr;
  // open the file
  char filepath[64] = "output_data_gpu_reducer/";
