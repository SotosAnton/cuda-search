...
__global__
void find_grid_loc_gpu(float*points,int*grid_loc,int n,int d,int k)
{
...
__global__ void search_block_multi_threaded(int *closest,float *current_min ,int block_offset,float *q, float *block ,int points_in_block){
...
__device__ void search_block_gpu(int *closest,float *current_min ,int block_offset,float *q, float *block ,int points_in_block){
...
__global__ void search_gpu(float*q,  float *c,  int*grid,int*points_per_block ,  int*closests,float*mindists  ,int n,int d,int dd)
{
  //int thread_id = blockIdx.x*blockDim.x + threadIdx.x;
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x;
   ...
   mindists[i]= (1<<10); //Inf
   search_block_gpu( &closests[i], &mindists[i] ,grid[ grid_loc ],&q[i*DIM], &c[ grid[ grid_loc ]*DIM ],points_per_block[grid_loc]);
     ...
     for(int dx=x-stage;dx<=x+stage;dx++){
       for(int dy=y-stage;dy<=y+stage;dy++){
         for(int dz=z-stage;dz<=z+stage;dz++){
           ...
           if(dx==x-stage||dx==x+stage||dy==y-stage||dy==y+stage||dz==z-stage||dz==z+stage){
             grid_loc= dx + d*dy + dd*dz;
             if(dx > -1 && dx < d && dy > -1 && dy < d && dz > -1 && dz < d )
             search_block_gpu( &closests[i], &mindists[i] ,grid[ grid_loc ],point, &c[ grid[ grid_loc ]*DIM ],points_per_block[grid_loc]);
           }
...
void find_grid_loc(float*points,int*grid_loc,int n,int d){
    int x,y,z;
    int dd = d*d;
   for(int i=0;i<n;i++){
   x=(int)(points[i*DIM+0]*d);
...
void search_block(int *closest,float *current_min ,int block_offset,float *q, float *block ,int points_in_block){
...
void search(float*q,  float *c,int*grid,int*points_per_block ,  int*closests,float*mindists  ,int N,int d)
{
 ...
 for(int i=0;i<N;i++){
  ...
  float per = (float) i / (float) N * 100;
   ...
   mindists[i] = (1<<10); //Inf
   search_block( &closests[i], &mindists[i] ,grid[ grid_loc ],point, &c[ grid[ grid_loc ]*DIM ],points_per_block[grid_loc]);
...
float* rearrange(float*p,int*intex,int* points_per_block,int* grid,int n,int k){

for (int i=0;i<k;i++) points_per_block[i]=0;
...
int main(int argc,char **argv)
{
  ...
  for(int i=1;i<=D;i++) intervals[i-1]=1/(float)D*i;
...
cudaMemcpy(d_c, c, N*DIM*sizeof(float), cudaMemcpyHostToDevice);
cudaMemcpy(points_per_block, points_block_c, block_num*sizeof(int), cudaMemcpyHostToDevice);
cudaMemcpy(block_loc, grid_c,block_num*sizeof(int), cudaMemcpyHostToDevice);
...
int fails=0;
 ...
 for(int x=0;x<D;x++){
    xmax=intervals[x];
    if(x==0){ xmin=0; }else{ xmin=intervals[x-1]; }
   for(int y=0;y<D;y++){
     ymax=intervals[y];
     if(x==0){ ymin=0; }else{ ymin=intervals[y-1]; }
      for(int z=0;z<D;z++){
       zmax=intervals[z];
       if(x==0){ zmin=0; }else{ zmin=intervals[z-1]; }
       pos = x + D*y + D*D*z;
       block_pos=grid_c[pos];

       for(int point=0;point<points_block_c[pos];point++){
        sum++;
        if(c[(block_pos+point)*DIM+0]>=xmax||c[(block_pos+point)*DIM+0]<xmin){
        fails++;
        //printf("fail at %d \n", block_pos );
        }
        if(c[(block_pos+point)*DIM+1]>=ymax||c[(block_pos+point)*DIM+1]<ymin){
        fails++;
        //printf("fail at %d \n", block_pos );
        }
        if(c[(block_pos+point)*DIM+2]>=zmax||c[(block_pos+point)*DIM+2]<zmin){
        fails++;
        //printf("fail at %d \n", block_pos );
        }
      }
...
search_gpu<<<blocks,threads_pblock>>>(d_q,  d_c, block_loc,points_per_block ,  d_closests,d_mindists  ,N,D,D*D);
cudaDeviceSynchronize();
...
for(int i=0;i<VALIDATE;i++){
   ...
   for(int j=0;j<N;j++)
   {
     dist  = (q[i*DIM+0]-c[j*DIM+0])*(q[i*DIM+0]-c[j*DIM+0]);
     dist += (q[i*DIM+1]-c[j*DIM+1])*(q[i*DIM+1]-c[j*DIM+1]);
     dist += (q[i*DIM+2]-c[j*DIM+2])*(q[i*DIM+2]-c[j*DIM+2]);
     dist = sqrtf(dist);
     if(dist<mindist){
       close=j;
       mindist=dist;
   ...
   if(close!=closest[i]){
   printf("intex %d %d dists %f %f  q :%f %f %f c: %f %f %f\n", close,closest[i],mindist,mindists[i],
                                                               q[i*DIM+0],q[i*DIM+1],q[i*DIM+2],
                                                               c[close*DIM+0],c[close*DIM+1],c[close*DIM+2]);
int x,y,z;
x=(int)(q[i*DIM+0]*D);
y=(int)(q[i*DIM+1]*D);
z=(int)(q[i*DIM+2]*D);
